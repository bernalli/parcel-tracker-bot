"""Yodel tracker (web scraping). Synthetic fixtures used for tests; selectors may need tuning at deploy time on real HTML."""
...
# British English status keywords for Yodel (UK domestic carrier).
...
class YodelTracker(AbstractTracker):
    """Yodel scraper (Tier S, priority 65)."""

    name: ClassVar[str] = "yodel"
    priority: ClassVar[int] = 65
    country_codes: ClassVar[list[str]] = ["GB"]
    tracking_id_patterns: ClassVar[list[re.Pattern[str]]] = [
        re.compile(r"^JD\d{16}$"),  # Yodel international (18 chars; DHL eCommerce uses JD\d{18})
        re.compile(r"^Y\d{14}$"),  # Yodel domestic
    ]
    url_patterns: ClassVar[list[re.Pattern[str]]] = [
        re.compile(r"yodel\.co\.uk/tracking", re.IGNORECASE),
    ]

    TRACK_URL: ClassVar[str] = "https://www.yodel.co.uk/tracking"
