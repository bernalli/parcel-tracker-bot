"""Bpost (Belgium) tracker (web scraping). Synthetic fixtures used for tests; selectors may need tuning at deploy time on real HTML."""
...
# Multi-locale status keywords (NL/FR/EN) for Bpost terminology (Belgium is trilingual).
_STATUS_KEYWORDS: dict[ShipmentStatus, tuple[str, ...]] = {
    ...
    ShipmentStatus.EXCEPTION: ("exception", "anomalie"),
    ShipmentStatus.ALERT: ("alert", "alerte"),
...
class BpostTracker(AbstractTracker):
    """Bpost scraper (Tier S, priority 60). Belgium national carrier."""

    name: ClassVar[str] = "bpost"
    priority: ClassVar[int] = 60
    country_codes: ClassVar[list[str]] = ["BE"]
    tracking_id_patterns: ClassVar[list[re.Pattern[str]]] = [
        re.compile(r"^[A-Z]{2}\d{9}BE$"),  # UPU BE
        re.compile(r"^32\d{16}$"),  # Bpost domestic 18-digit (32 prefix)
    ]
    url_patterns: ClassVar[list[re.Pattern[str]]] = [
        re.compile(r"track\.bpost\.cloud", re.IGNORECASE),
    ]

    TRACK_URL: ClassVar[str] = "https://track.bpost.cloud/btr/web/"
    ...
    def _map_status(raw: str) -> ShipmentStatus:
        ...
        # out-for-delivery phrase (e.g. "livré" is a substring of "en cours de livraison"
        # if matched naively).
        for status in (
            ShipmentStatus.OUT_FOR_DELIVERY,
            ShipmentStatus.DELIVERED,
