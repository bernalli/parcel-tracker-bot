"""Australia Post tracker (web scraping). Synthetic fixtures used for tests; selectors may need tuning at deploy time on real HTML."""
...
# Multi-locale status keywords (EN primary for Australia Post, plus DE/IT/PT/FR/ES) for cross-border shipments.
...
class AustraliaPostTracker(AbstractTracker):
    """Australia Post scraper (Tier S, priority 80)."""

    name: ClassVar[str] = "australia_post"
    priority: ClassVar[int] = 80
    country_codes: ClassVar[list[str]] = ["AU"]
    ...
    TRACK_URL: ClassVar[str] = "https://auspost.com.au/mypost/track/details"
    ...
    async def fetch(self, tracking_id: str) -> TrackingResult:
        ...
        try:
            ...
            logger.warning(
                "Australia Post fetch failed for %s: %s",
                normalized,
                exc,
