"""Canada Post tracker (web scraping). Synthetic fixtures used for tests; selectors may need tuning at deploy time on real HTML."""
...
# Multi-locale status keywords (EN primary for Canada Post, plus FR/DE/IT/PT/ES) for cross-border shipments.
...
class CanadaPostTracker(AbstractTracker):
    """Canada Post scraper (Tier S, priority 80)."""

    name: ClassVar[str] = "canada_post"
    priority: ClassVar[int] = 80
    country_codes: ClassVar[list[str]] = ["CA"]
    tracking_id_patterns: ClassVar[list[re.Pattern[str]]] = [
        re.compile(r"^\d{16}$"),  # Canada Post domestic 16-digit
        re.compile(r"^[A-Z]{2}\d{9}CA$"),  # UPU CA
    ]
    url_patterns: ClassVar[list[re.Pattern[str]]] = [
        re.compile(r"canadapost-postescanada\.ca/track", re.IGNORECASE),
    ]

    TRACK_URL: ClassVar[str] = "https://www.canadapost-postescanada.ca/track-reperage/en"
    ...
    async def fetch(self, tracking_id: str) -> TrackingResult:
        ...
        try:
            ...
            logger.warning(
                "Canada Post fetch failed for %s: %s",
                normalized,
                exc,
