"""Royal Mail tracker (web scraping). Synthetic fixtures used for tests; selectors may need tuning at deploy time on real HTML."""
...
# Multi-locale status keywords (EN/IT/PT/FR/DE/ES) for Royal Mail terminology (British English).
...
class RoyalMailTracker(AbstractTracker):
    """Royal Mail scraper (Tier S, priority 85)."""
    ...
    tracking_id_patterns: ClassVar[list[re.Pattern[str]]] = [
        re.compile(r"^[A-Z]{2}\d{9}GB$"),  # UPU GB-suffix (Royal Mail international)
    ]
    url_patterns: ClassVar[list[re.Pattern[str]]] = [
        re.compile(r"royalmail\.com.*track", re.IGNORECASE),
    ]

    TRACK_URL: ClassVar[str] = "https://www.royalmail.com/track-your-item"
    ...
    def _parse_html(self, tracking_id: str, html: str) -> TrackingResult:
        ...
        status_el = soup.select_one(
            ".tracking-status, .delivery-status, .package-status, .shipment-status"
        )
