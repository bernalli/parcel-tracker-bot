"""USPS tracker (web scraping). Synthetic fixtures used for tests; selectors may need tuning at deploy time on real HTML."""
...
# Multi-locale status keywords (EN/IT/PT/FR/DE/ES) for USPS terminology.
...
class UspsTracker(AbstractTracker):
    """USPS scraper (Tier S, priority 90)."""

    name: ClassVar[str] = "usps"
    ...
    url_patterns: ClassVar[list[re.Pattern[str]]] = [
        re.compile(r"tools\.usps\.com.*tLabels=", re.IGNORECASE),
    ]

    TRACK_URL: ClassVar[str] = "https://tools.usps.com/go/TrackConfirmAction"
    ...
    def _parse_html(self, tracking_id: str, html: str) -> TrackingResult:
        ...
        for row in soup.select(".tracking-event, tr.tracking-event, .tracking-history tr"):
        ...
        status_el = soup.select_one(
            ".tracking-status, .package-status, .shipment-status, .current-status"
        )
