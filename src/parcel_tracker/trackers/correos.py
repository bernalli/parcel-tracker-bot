"""Correos (Spain) tracker (web scraping). Synthetic fixtures used for tests; selectors may need tuning at deploy time on real HTML."""
...
# Multi-locale status keywords (ES primary for Correos, plus EN/FR/DE/IT/PT) for cross-border shipments.
_STATUS_KEYWORDS: dict[ShipmentStatus, tuple[str, ...]] = {
    ...
    ShipmentStatus.ALERT: ("alerta", "alert", "warnung", "avviso", "alerte"),
...
class CorreosTracker(AbstractTracker):
    """Correos (Spain) scraper (Tier S, priority 75)."""

    name: ClassVar[str] = "correos"
    priority: ClassVar[int] = 75
    country_codes: ClassVar[list[str]] = ["ES"]
    tracking_id_patterns: ClassVar[list[re.Pattern[str]]] = [
        re.compile(r"^[A-Z]{2}\d{9}ES$"),  # UPU ES
    ]
    url_patterns: ClassVar[list[re.Pattern[str]]] = [
        re.compile(r"correos\.es.*localizador", re.IGNORECASE),
    ]

    TRACK_URL: ClassVar[str] = "https://www.correos.es/es/es/herramientas/localizador"
