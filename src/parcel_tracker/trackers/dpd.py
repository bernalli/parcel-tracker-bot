"""DPD tracker (web scraping). Synthetic fixtures used for tests; selectors may need tuning at deploy time on real HTML."""
...
# Multi-locale status keywords (DE primary for DPD, plus EN/FR/IT/PT/ES) for cross-border shipments.
...
class DpdTracker(AbstractTracker):
    """DPD scraper (Tier S, priority 70)."""

    name: ClassVar[str] = "dpd"
    priority: ClassVar[int] = 70
    country_codes: ClassVar[list[str]] = ["DE", "GB", "FR", "GLOBAL"]
    tracking_id_patterns: ClassVar[list[re.Pattern[str]]] = [
        re.compile(r"^\d{14}$"),  # DPD 14-digit
        re.compile(r"^\d{17}$"),  # DPD 17-digit
    ]
    url_patterns: ClassVar[list[re.Pattern[str]]] = [
        re.compile(r"dpd\.(de|com|co\.uk).*tracking", re.IGNORECASE),
    ]

    TRACK_URL: ClassVar[str] = "https://tracking.dpd.de/parcelstatus"
