...
def test_disputed_stops_after_grace_window() -> None:
    ...
    assert is_due(ShipmentStatus.DELIVERED, now - timedelta(hours=2), now,
                  delivery_disputed=True, delivered_at=old_delivery) is False
...
def test_disputed_within_grace_still_due() -> None:
    ...
    assert is_due(ShipmentStatus.DELIVERED, now - timedelta(hours=2), now,
                  delivery_disputed=True, delivered_at=recent_delivery) is True
