...
@pytest.mark.asyncio
async def test_delivered_transition_sets_delivered_and_prompts() -> None:
    ...
    result = TrackingResult(tracking_number="FAKE1", found=True,
                            status=ShipmentStatus.DELIVERED, last_event="Delivered",
                            last_location="Milano, Italy", events=[])
    detector = MagicMock(); detector.detect.return_value = [_T(result)]
    repo = MagicMock()
    repo.list_active_for_user = AsyncMock(return_value=[parcel])
    repo.set_last_check_at = AsyncMock(); repo.update_status = AsyncMock()
    repo.add_events_dedup = AsyncMock(return_value=[]); repo.update_latest = AsyncMock()
    repo.set_delivered = AsyncMock()
    user_repo = MagicMock(); user_repo.get_allowed_user_ids = AsyncMock(return_value=[])
    health = MagicMock()
    health.is_quarantined = AsyncMock(return_value=False)
    health.record_success = AsyncMock(); health.record_failure = AsyncMock()
    notifier = MagicMock(); notifier.send_delivery_confirmation = AsyncMock()
    notifier.send_events_update = AsyncMock()
    config = MagicMock(); config.batch_size = 10; config.owner_id = 7; config.allowed_user_ids = []
    prefs = MagicMock(); prefs.is_status_enabled = AsyncMock(return_value=True)
    ctx = MagicMock()
    ...
    repo.set_delivered.assert_awaited_once()
    notifier.send_delivery_confirmation.assert_awaited_once()
