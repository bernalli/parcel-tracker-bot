...
| `usps` | 90 | US | `^91\d{20}$` / `^94\d{20}$` / `^9\d{19}$` / `^E[A-Z]\d{9}US$` (added `^94\d{20}$` in item 30 tuning) |
| `royal_mail` | 85 | GB | `^[A-Z]{2}\d{9}GB$` |
| `la_poste` | 85 | FR | `^[A-Z]{2}\d{9}FR$` / `^[A-Z0-9]{11,13}FR$` |
| `deutsche_post` | 85 | DE | `^[A-Z]{2}\d{9}DE$` / `^\d{20}$` |
| `aramex` | 80 | AE, GLOBAL | `^\d{11}$` (tightened in item 30 — was 10-12) |
...
| `dhl` | 70 | DE, GLOBAL | `^\d{10}$` / `^JD\d{18}$` / `^(?!TBA)[A-Z]{3}\d{9,12}$` (negative lookahead `(?!TBA)` added in item 30 to prevent collision with Amazon Logistics) |
| `fedex` | 70 | US, GLOBAL | `^\d{12}$` / `^\d{15}$` / `^\d{20}$` / `^\d{22}$` / `^GD\d{9}$` / `^[A-Z]{2}\d{9}TN$` |
| `dpd` | 70 | DE, GB, FR, GLOBAL | `^\d{14}$` / `^\d{17}$` |
| `gls_europe` | 70 | DE, AT, BE, NL, GLOBAL | `^\d{11}$` / `^\d{12}$` / `^\d{13}$` / `^\d{14}$` (added `^\d{13}$` in item 30) |
| `yodel` | 65 | GB | `^JD\d{16}$` / `^Y\d{14}$` |
| `evri` | 65 | GB | `^H\d{15}$` / `^T\d{16}$` / `^\d{16}$` (Hermes rebrand 2022) |
| `bpost` | 60 | BE | `^[A-Z]{2}\d{9}BE$` / `^32\d{16}$` |
| `postnl` | 60 | NL | `^3S[A-Z0-9]{9,13}$` / `^[A-Z]{2}\d{9}NL$` / `^LL\d{9}NL$` (tail widened to `{9,13}` in item 30 — was `{9,11}`) |
...
| `china_post` | 35 | CN | China Post | `^[LRCES][A-Z]\d{9}CN$` (tightened to `[LRCES]` prefix in item 25) |
