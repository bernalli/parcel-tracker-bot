...
| `dhl` | 70 | DE, GLOBAL | `^\d{10}$` / `^JD\d{18}$` / `^(?!TBA)[A-Z]{3}\d{9,12}$` (negative lookahead `(?!TBA)` added to prevent collision with Amazon Logistics) |
| `fedex` | 70 | US, GLOBAL | `^\d{12}$` / `^\d{15}$` / `^\d{20}$` / `^\d{22}$` / `^GD\d{9}$` / `^[A-Z]{2}\d{9}TN$` |
| `dpd` | 70 | DE, GB, FR, GLOBAL | `^\d{14}$` / `^\d{17}$` |
| `gls_europe` | 70 | DE, AT, BE, NL, GLOBAL | `^\d{11}$` / `^\d{12}$` / `^\d{13}$` / `^\d{14}$` (added `^\d{13}$`) |
...
| `china_post` | 35 | CN | China Post | `^[LRCES][A-Z]\d{9}CN$` (tightened to `[LRCES]` prefix) |
